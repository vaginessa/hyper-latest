...
function downloadsView (req, res, { release, isLatestVersion }) {
  ...
  const repoLink = render('a', { class: 'a--repolink', href: `https://github.com/${REPO_PATH}` }, repoHeader())
  ...
  return [
    flexBox([render('h2', null, repoLink), timeStamp]),
    content
  ].join('')
}
...
async function versionsView (req, res, releases) {
  ...
  const repoLink = render('a', { class: 'a--repolink text--thin', href: `https://github.com/${REPO_PATH}` }, repoHeader())
...
function releaseView (req, res, { release, isLatestVersion }) {
  ...
  const title = render('h2', { class: 'text--thin' }, `${versionTag} ${repoHeader()} ${latestTag} release notes`)
